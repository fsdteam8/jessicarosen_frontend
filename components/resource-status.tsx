...
export function ResourceStatus() {
  ...
  const [selectedResourceId, setSelectedResourecId] = useState<string | null>(
    null
  );
    ...
    queryFn: async () => {
      ...
      if (!response.ok) {
        const errorData = await response
          .json()
          .catch(() => ({ message: "Failed to fetch resources" }));
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8 text-orange-600 bg-orange-50 rounded-lg shadow">
        ...
        <p className="text-sm text-gray-500 mt-2">
          Currently using fallback: {baseUrl}
        </p>
      </div>
    );
  }
            ...
            <tbody>
              {paginatedResources.length === 0 && !isLoading ? (
                ...
                paginatedResources.map((resource) => (
                  <tr
                    key={resource._id}
                    className="border-b border-[#B6B6B6] hover:bg-gray-50 bg-white"
                  >
                    <td className="px-6 py-3">
                      <div className="flex items-center space-x-3">
                        <Image
                          src={
                            resource.thumbnail ||
                            "/placeholder.svg?height=60&width=60&query=resource+thumbnail"
                          }
                          alt={resource.title}
                          width={60}
                          height={60}
                        ...
                        />
                        <span
                          className="font-medium text-[#424242] text-sm max-w-xs truncate"
                    ...
                    <td
                      className="font-medium text-[#424242] text-sm px-4 py-3 truncate"
                      title={resource._id}
                    >
                      {resource._id}
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center space-x-1">
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  ...
                                  permanently delete the resource titled &quot;
                                  {resource.title}&quot;.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
                        ...
                        <Button
                          ...
                          onClick={() => {
                            setIsChatOpen(true);
                            setSelectedResourecId(resource?._id);
                          }}
                    ...
                    <td className="px-4 py-3">
                      <Badge
                        className={`${getStatusColor(resource.status)} text-xs`}
                      >
                        {resource.status || "N/A"}
                      </Badge>
                    </td>
