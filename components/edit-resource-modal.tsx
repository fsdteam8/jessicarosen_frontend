...
export function EditResourceModal({
  ...
  useEffect(() => {
    if (resource && countriesData && practiceAreasData && resourceTypesData) {
      setFormData({
        ...
        practiceArea:
          practiceAreasData.find((p) =>
            resource.practiceAreas?.includes(p.name)
          )?._id || "",
        resourceType:
          resourceTypesData.find((rt) =>
            resource.resourceType?.includes(rt.resourceTypeName)
          )?._id || "",
      ...
      const countryObj = countriesData.find(
        (c) => c.countryName === resource.country
      );
  ...
  useEffect(() => {
    ...
    return () => {
      if (
        ...
        currentThumbnailPreview.startsWith("blob:")
      ) {
  ...
  const { mutate: updateResource, isPending: isSubmitting } = useMutation({
    mutationFn: async (currentFormData: FormDataState) => {
      ...
      const uploadFileAndGetUrl = async (
        ...
        return `https://res.cloudinary.com/your_cloud/${
          type === "file" ? "raw" : "image"
        }/upload/v_placeholder/new_${type}_${Date.now()}_${file.name}`; // Placeholder URL
      };
      ...
      if (practiceAreaObj)
        metadataUpdatePayload.practiceAreas = [practiceAreaObj.name];
      else if (currentFormData.practiceArea)
        metadataUpdatePayload.practiceAreas = [currentFormData.practiceArea];

      const resourceTypeObj = resourceTypesData?.find(
        (rt) => rt._id === currentFormData.resourceType
      );
      if (resourceTypeObj)
        metadataUpdatePayload.resourceType = [resourceTypeObj.resourceTypeName];
      else if (currentFormData.resourceType)
        metadataUpdatePayload.resourceType = [currentFormData.resourceType];

      // --- Thumbnail Handling ---
      if (currentFormData.thumbnail instanceof File) {
        metadataUpdatePayload.thumbnail = await uploadFileAndGetUrl(
      ...
      if (currentFormData.file instanceof File) {
        ...
        metadataUpdatePayload.file = {
          url: fileUrl,
          type: currentFormData.file.type,
        };
      } else if (
        currentFormData.file === null &&
        currentFormData.existingFileName === null &&
        resource.file?.url
      ) {
        // Explicitly removed
        ...
        metadataUpdatePayload.file = {
          url: currentFormData.existingFileUrl,
  ...
  const handleInputChange = (
    ...
    value: string | string[] | (File | string)[]
  ) => {
  ...
  const handleThumbnailChange = (
    ...
    if (
      ...
      formData.thumbnailPreview.startsWith("blob:")
    ) {
    ...
    if (file) {
      if (!file.type.startsWith("image/")) {
        ...
        setFormData((prev) => ({
          ...prev,
          thumbnail: null,
          thumbnailPreview: prev.existingThumbnailUrl,
        }));
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      file: file,
      existingFileName: file ? file.name : prev.existingFileName,
    }));
    if (!file && fileInputRef.current) {
      ...
      setFormData((prev) => ({
        ...prev,
        existingFileName:
          resource?.file?.url?.substring(
            resource.file.url.lastIndexOf("/") + 1
          ) || null,
      }));
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 max-h-[80vh] overflow-y-auto p-4">
              ...
              <CardContent>
                ...
                {formData.imagePreviews && formData.imagePreviews.length > 0 ? (
                  ...
                  <p className="text-sm text-muted-foreground">
                    No images uploaded yet. Click above to add some.
                  </p>
                )}
              </CardContent>
            </Card>
          ...
          <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        <Image
                          ...
                          className="max-h-32 w-auto mx-auto rounded-md object-contain opacity-75 hover:opacity-100 transition-opacity"
                        />
                      </label>
                      {formData.thumbnail && (
                        <p
                      ...
                      <label
                        htmlFor="thumbnail-upload-edit"
                        className="cursor-pointer inline-block text-xs text-blue-600 hover:text-blue-800 underline"
                      >
            ...
            <Card>
              ...
              <CardContent className="pt-0 p-4">
                <div className="border-2 border-dashed rounded-lg p-4 text-center space-y-2 cursor-pointer opacity-100">
                  <input
                    type="file"
                  ...
                  {formData.file ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <p
                        className="text-xs text-gray-500 truncate"
                        title={formData.existingFileName}
                      >
