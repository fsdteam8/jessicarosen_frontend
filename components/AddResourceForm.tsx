...
export default function ResourceForm() {
  ...
  const thumbnailImages = useMemo(
    () => ["/assets/bs1.png", "/assets/bs2.png", "/assets/hh.png"],
    []
  );
  ...
  const [formData, setFormData] = useState<FormDataState>({
    ...
    practiceArea: "",
    resourceType: "",
    thumbnail: thumbnailImages[0], // Default to first image
    file: null,
    images: [],
  });
  ...
  useEffect(() => {
    setIsClient(true);
    setThumbnailPreview(thumbnailImages[0]);
  }, [thumbnailImages]);
  ...
  const { mutate: submitResource } = useMutation({
    mutationFn: async (currentFormData: FormDataState) => {
      ...
      if (typeof currentFormData.thumbnail === "string") {
        submitData.append("thumbnailUrl", currentFormData.thumbnail);
      } else {
  ...
  const handleThumbnailUpload = (
    ...
    if (file) {
      if (!file.type.startsWith("image/")) {
        ...
        setFormData((prev) => ({ ...prev, thumbnail: thumbnailImages[0] }));
        setThumbnailPreview(thumbnailImages[0]);
        if (thumbnailInputRef.current) {
          thumbnailInputRef.current.value = "";
        }
  ...
  const handleSubmit = (action: "publish" | "draft") => {
    ...
    const dataToLog = {
      ...
      resourceType: resourceTypeObj
        ...
        : [],
      thumbnail:
        typeof formDataToSubmit.thumbnail === "string"
          ? formDataToSubmit.thumbnail
