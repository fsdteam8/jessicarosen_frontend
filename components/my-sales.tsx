...
export function MySales() {
  ...
  const fetchAllSales = async (): Promise<SalesData[]> => {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/seller/dashboard/my-sales`, {
      method: "GET",
      headers: {
  ...
  const searchSales = async (query: string): Promise<SalesData[]> => {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/seller/dashboard/my-sales?search=${encodeURIComponent(query)}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const displayData = isSearching && searchTerm.trim() ? searchResults : allSalesData
  const isLoading = isSearching && searchTerm.trim() ? isLoadingSearch : isLoadingAll
  ...
  const totalSales = allSalesData.reduce((sum, item) => sum + item.totalSellAmount, 0)
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex items-center space-x-2">
          ...
          <span>{isSearching ? "Searching sales data..." : "Loading sales data..."}</span>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        <div className="text-red-600 text-center">
          <p className="font-semibold">{isSearching ? "Error searching sales data" : "Error loading sales data"}</p>
          ...
          <div className="flex flex-col gap-4">
            <CardTitle className="text-lg font-semibold">
              ...
              {isSearching && searchTerm && (
                <span className="text-sm font-normal text-gray-600 ml-2">- Search results for {searchTerm}</span>
              )}
            </CardTitle>
            <div className="flex items-center space-x-2">
        ...
        <CardContent className="bg-[#EDEEF1]">
          {displayData.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              {isSearching && searchTerm ? "No sales found matching your search." : "No sales data available."}
            </div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="border-b">
                    <tr>
                      <th className="text-left px-[50px] font-medium text-[#131313]">Product ID</th>
                      <th className="text-left p-4 font-medium text-[#131313]">Quantity</th>
                      <th className="text-left p-4 font-medium text-[#131313]">Total Sell Amount</th>
                      <th className="text-left p-4 font-medium text-[#131313]">
                        Admin Charge <span className="text-orange-500">(50%)</span>
                      </th>
                      <th className="text-left p-4 font-medium text-[#131313]">Seller Revenue</th>
                    </tr>
                  </thead>
                  <tbody>
                    {paginatedData.map((item, index) => (
                      <tr key={`${item.productId}-${index}`} className="border-b hover:bg-gray-50">
                        ...
                        <td className="p-4 text-[#424242]">${item.totalSellAmount.toFixed(2)}</td>
                        <td className="p-4 text-[#424242]">${item.adminCharge.toFixed(2)}</td>
                        <td className="p-4 text-[#424242]">${item.sellerRevenue.toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                <span className="text-sm text-gray-600">
                  ...
                  {Math.min(currentPage * itemsPerPage, displayData.length)} of {displayData.length} results
                </span>
